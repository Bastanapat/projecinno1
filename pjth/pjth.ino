...
void callback(char* topic, byte* payload, unsigned int length){
  ...
  std::string tp = topic;
  if(tp == "innoproject/group2/control"){
    int control_start_stop = docd["control"];
//    Serial.println(control_start_stop);
  ...
  if(tp == "innoproject/group2/delay"){
     int de = docd["delay"];
     Serial.println(de);
     if (de == 1){
      delay1 = 1000;
...
void loop() {
  if (mqtt.connected() == false) {
    ...
    if (mqtt.connect(MQTT_NAME, MQTT_USERNAME, MQTT_PASSWORD)) 
    {
      ...
      mqtt.subscribe("innoproject/group2/control");
      mqtt.subscribe("innoproject/group2/delay");
    } 
    else {
...
//        Serial.println(celsiusTemp);                 
       /////////////////////////////////////////////////
          mqtt.loop();
          docs1["Humidity"] = humidity;
          docs1["temperature"] = celsiusTemp;
          serializeJson(docs1,output1);
          if(st_sp == "on"){
          mqtt.publish("innoproject/group2/measure",output1);
                           }
          else if(st_sp == "off"){
          mqtt.publish("innoproject/group2/measure",".......");
                           }
          delay(delay1);  
       }
            }
